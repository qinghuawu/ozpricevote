$(document).ready(function () {
    ...
    $(window).bind('hashchange', function () {
        ...
        if (hash === 'login') {
            url = login_url
        } else if (hash === 'my-space') {
            url = user_space_page_url
        } else if (hash == 'discount') {
            url = discount_page_url
        } else if (hash == 'like-chart') {
            url = like_chart_page_url
        } else if (hash == 'signup') {
            url = signup_url
        } else if (hash == 'my-like'){
            url = my_like_page_url
        } else if (hash == 'my-collection'){
            url = my_collection_page_url
        } else if (hash == 'cheapest-in-history'){
            url = cheapest_page_url
        } else {
            return;
        }
    ...
    if (window.location.hash === '') {
        window.location.hash = '#discount'; // home page, show the default view
    } else {
    ...
    function user_space() {
        ...
        $.ajax({
            ...
            contentType: 'application/json;charset=UTF-8',
            success: function (data) {
                $('#main').hide().html(data.html).fadeIn(800);
                window.location.hash = '#search-res';
                activeM();
            }
        });
    }
    ...
    $(document).on('click', '#signup-btn', register);
    ...
    function login_user() {
        ...
        $.ajax({
            ...
            contentType: 'application/json;charset=UTF-8',
            success: function (data) {
                if (window.location.hash === '#discount' || window.location.hash === 'discount') {
                    $(window).trigger('hashchange');
                } else {
                    window.location.hash = '#discount';
                }
                activeM();
                M.toast({html: data.message});
            }
